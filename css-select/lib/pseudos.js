        ...
        return function nthChild(elem) {
            ...
            for (var i = 0, pos = 0; i < siblings.length; i++) {
                if (adapter.isTag(siblings[i])) {
                    if (adapter.equals(siblings[i],elem)) break;
                    else pos++;
                }
            }
...
var pseudos = {
    ...
    "first-child": function(elem, adapter) {
        return adapter.equals(
			getFirstElement(adapter.getSiblings(elem), adapter),
			elem
		);
    },
    "last-child": function(elem, adapter) {
        ...
        for (var i = siblings.length - 1; i >= 0; i--) {
            if (adapter.equals(siblings[i],elem)) return true;
    ...
    "only-of-type": function(elem, adapter) {
        ...
        for (var i = 0, j = siblings.length; i < j; i++) {
            if (adapter.isTag(siblings[i])) {
                if (adapter.equals(siblings[i],elem)) continue;
    ...
    selected: function(elem, adapter) {
        ...
        for (var i = 0; i < siblings.length; i++) {
            if (adapter.isTag(siblings[i])) {
                if (adapter.equals(siblings[i],elem)) {
                    sawElem = true;
                } else if (!sawElem) {
                    return false;
